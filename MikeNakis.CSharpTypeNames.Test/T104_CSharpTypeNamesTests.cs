...
public sealed class T104_CSharpTypeNamesTests
{
	...
	public void Test()
	{
		...
		{
			//PEARL: Microsoft.CSharp.CSharpCodeProvider.GetTypeOutput() generates type names in a somewhat capricious
			//    style, so we have to match that style here in order to successfully compare type names.
			string generatedTypeName = Generator.GetCSharpTypeName( type, Options.UseLanguageKeywordsForBuiltInTypes );
